...
import { Package, FileText, Calculator, Search, Plus, Building2, Download, Edit3, Trash2, ImagePlus, Save, X } from "lucide-react";
import "./style.css";
...
function App() {
  ...
  function saveItem() {
    ...
    if (editingId) {
      setItems(items.map((x) => x.id === editingId ? {
        ...x, ...form,
  ...
  function editItem(item) {
    setForm({ ...emptyForm, ...item, images: item.images || [] });
  ...
  const menu = [["dashboard", "控制台"], ["add", editingId ? "编辑商品" : "商品录入"], ["inventory", "库存管理"], ["ledger", "古物台账"], ["customs", "EMS报关"], ["profit", "利润分析"]];

  return <div className="app">
    <aside>
      <div className="brand"><Building2 size={24}/><div><b>豪嘉株式会社</b><span>GOUKA Luxury ERP V2</span></div></div>
      {menu.map(([k, v]) => <button key={k} className={tab === k ? "active" : ""} onClick={() => setTab(k)}>{v}</button>)}
    </aside>
    <main>
      <header><div><h1>二手奢侈品管理系统 V2</h1><p>编辑・删除・自动保存・图片上传・状态筛选・古物台账・EMS报关・利润计算</p></div><span className="pill">Auto Save</span></header>
      {tab === "dashboard" && <Dashboard totals={totals} items={items}/>} 
      {tab === "add" && <AddForm form={form} setForm={setForm} saveItem={saveItem} resetForm={resetForm} editingId={editingId} handleImages={handleImages} removeImage={removeImage}/>} 
      {tab === "inventory" && <Inventory items={filtered} query={query} setQuery={setQuery} statusFilter={statusFilter} setStatusFilter={setStatusFilter} downloadCSV={downloadCSV} editItem={editItem} deleteItem={deleteItem}/>} 
...
function Dashboard({ totals, items }) {
  ...
  return <section className="grid4">
    ...
    <div className="panel wide"><h2>系统说明</h2><p>V2 已支持：编辑、删除、自动保存、本地图片上传、状态筛选、CSV导出。</p><p>注意：当前图片保存在本浏览器里。以后升级 Supabase 后，可支持员工多电脑同步。</p><p>预计利润率：<b>{margin.toFixed(1)}%</b></p></div>
  </section>;
}
...
function AddForm({ form, setForm, saveItem, resetForm, editingId, handleImages, removeImage }) {
  ...
  return <div className="panel"><h2><Plus size={20}/> {editingId ? `编辑商品：${editingId}` : "新增商品"}</h2>
    <div className="formgrid">
      ...
      <label className="full">商品图片（最多3张，本地保存）<input type="file" accept="image/*" multiple onChange={(e) => handleImages(e.target.files)}/></label>
      <div className="image-row full">{(form.images || []).map((src, i) => <div className="image-box" key={i}><img src={src} alt={`商品图片${i + 1}`}/><button type="button" onClick={() => removeImage(i)}><X size={14}/> 移除</button></div>)}</div>
      <label className="full">备注<textarea value={form.memo} onChange={(e) => set("memo", e.target.value)}/></label>
    </div>
    <div className="action-row"><button className="primary" onClick={saveItem}><Save size={16}/> {editingId ? "保存修改" : "添加到库存"}</button>{editingId && <button className="ghost" onClick={resetForm}>取消编辑</button>}</div>
  </div>;
}
...
function Inventory({ items, query, setQuery, statusFilter, setStatusFilter, downloadCSV, editItem, deleteItem }) {
  ...
  const rows = items.map((x) => [
    x.images && x.images.length ? <img className="thumb" src={x.images[0]} alt={x.item}/> : "—", x.id, x.purchaseDate, x.category, x.brand, x.item, x.material, x.color, x.origin, x.qty, x.purchaseCny, x.declaredCny, x.saleJpy, x.status,
    <div className="table-actions"><button className="edit" onClick={() => editItem(x)}><Edit3 size={14}/> 编辑</button><button className="danger" onClick={() => deleteItem(x.id)}><Trash2 size={14}/> 删除</button></div>
  ]);
  return <div className="panel"><Toolbar title="库存管理" query={query} setQuery={setQuery} statusFilter={statusFilter} setStatusFilter={setStatusFilter} onDownload={() => downloadCSV(csvRows, "gouka_inventory.csv")}/><Table headers={headers} rows={rows}/></div>;
}
...
function Ledger({ items, downloadCSV }) {
  ...
  return <div className="panel"><Toolbar title="古物台账" onDownload={() => downloadCSV([headers, ...rows], "gouka_kobutsu_ledger.csv")}/><Table headers={headers} rows={rows}/></div>;
}
...
function Customs({ items, downloadCSV }) {
  ...
  return <div className="panel"><Toolbar title="EMS Commercial Customs Declaration" onDownload={() => downloadCSV([headers, ...rows, [], ["Total Quantity", totalQty], ["Total Declared Value", totalValue]], "gouka_ems_customs.csv")}/><p><b>Importer:</b> 豪嘉株式会社 (GOUKA INC.)</p><Table headers={headers} rows={rows}/><p className="note">Total Quantity: {totalQty} pieces / Total Declared Value: {cny(totalValue)}</p></div>;
}

function Profit({ items }) {
  const headers = ["商品编号", "品牌", "商品名", "采购成本JPY", "预计销售JPY", "预计毛利JPY", "利润率"];
  const rows = items.map((x) => { const cost = Number(x.purchaseCny || 0) * Number(x.rate || 0); const profit = Number(x.saleJpy || 0) - cost; const margin = x.saleJpy ? ((profit / x.saleJpy) * 100).toFixed(1) + "%" : ""; return [x.id, x.brand, x.item, Math.round(cost), x.saleJpy, Math.round(profit), margin]; });
  return <div className="panel"><h2><Calculator size={20}/> 利润分析</h2><Table headers={headers} rows={rows}/></div>;
}
...
function Toolbar({ title, query, setQuery, statusFilter, setStatusFilter, onDownload }) {
  ...
  return <div className="toolbar"><h2>{title}</h2><div className="toolbar-right">{setQuery && <div className="search"><Search size={16}/><input placeholder="搜索品牌/商品/材质" value={query} onChange={(e) => setQuery(e.target.value)}/></div>}{setStatusFilter && <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>{statuses.map((s) => <option key={s}>{s}</option>)}</select>}{onDownload && <button onClick={onDownload}><Download size={16}/> CSV导出</button>}</div></div>;
}
