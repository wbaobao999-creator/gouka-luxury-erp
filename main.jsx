...
function App() {
  ...
  function exportBackup() {
    const data = { version: "GOUKA-ERP-ENTERPRISE-1.1-LISTING-STABLE", exportedAt: new Date().toISOString(), items, customsBatches, dictionaries, suppliers, deleteLogs };
...
<div class="pdf-page">${bodyHtml}</div>
  ...
  function exportItemPdf(item) {
    ...
    const body = `
      <div class="pdf-header">
        <div>
          <h1>商品资料 PDF</h1>
          <p>Product Sheet / GOUKA ERP Enterprise 1.2</p>
        </div>
        <div class="company">
          <b>豪嘉株式会社</b><br />
          GOUKA INC.<br />
  ...
  function exportInventoryPdf(targetItems = null, label = "库存报告") {
    ...
    const body = `
      ...
      <div class="footer">GOUKA ERP Enterprise 1.2 / Generated by GOUKA ERP. 金额与消费税为内部参考，正式申告请交由税理士确认。</div>
    `;
    ...
    <div className="app">
      <aside>
        <div className="brand">
          ...
          <div>
            <b>豪嘉株式会社</b>
            <span>GOUKA ERP Enterprise 1.2</span>
          </div>
        </div>
      ...
      <main>
        <header>
          <div>
            <h1>豪嘉ERP Enterprise 1.2 Auction Settlement</h1>
  ...
  return (
    <section className="v3-dashboard">
      <div className="v3-hero">
        <div>
          <span className="v3-kicker">GOUKA ERP Enterprise 1.2</span>
          <h1>经营驾驶舱 · Enterprise 1.1</h1>
      ...
      <div className="panel wide">
        <h2>经营提醒</h2>
        <p>V7.11新增今日经营、库存预警、品牌利润排行、供应商利润排行。Enterprise 1.1：优化出品管理优先级、统一图片尺寸、菜单按日常业务重新排序；保留云端同步、PDF、古物台账和全页面图片。</p>
      </div>
    </section>
  );
}
...
function AuctionSettlementBox({ form, setForm }) {
  ...
  const taxExcludedCost = hammer + buyerFee + domesticShipping;
  ...
  return (
    <div className="full panel" style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", padding: "16px" }}>
      ...
      <p className="note">按结算单填写。系统会自动把采购币种改为 JPY，汇率改为 1，并把落札金額、手续费写入成本。消费税金额会写入备注，给税理士参考。</p>
      ...
      <div className="grid4" style={{ marginTop: "12px" }}>
        ...
        <Card icon={<Calculator />} title="税抜成本参考" value={jpy(taxExcludedCost)} />
      ...
      <p className="note">例：44,000 + 4,400 + 440 + 44 = 48,884。录入时落札金額填 44,000，税额写入备注；实际税务处理以税理士为准。</p>
    </div>
  );
}
...
function ListingManagement({ items, updateListingItem, editItem, setPreviewImage, setPreviewScale }) {
  ...
  return (
    <div className="panel">
      ...
      <p className="note">Enterprise 1.1：出品管理优先优化。平台支持 NBAA / OBA / ECO Ring / JBA / AUCNET / Star Buyers / Mercari / Yahoo / 楽天 / 店铺，也可以手动输入。可快速把已入库商品加入待出品。</p>
