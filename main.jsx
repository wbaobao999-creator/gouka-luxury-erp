  ...
  function companyPdfHeader(title, subtitle = "Inventory Report") {
    ...
    return `
      ...
      <div class="report-title enterprise-title">
        ...
        <div class="erp-version">GOUKA ERP Enterprise 4.0</div>
      </div>
    `;
  }
...
<div class="pdf-page">${bodyHtml}</div>
  ...
  function exportInventoryPdf(targetItems = null, label = "库存报告") {
    ...
    const body = `
      ...
      <div class="footer">GOUKA ERP Enterprise 4.0 / Generated by GOUKA ERP. 金额与消费税为内部参考，正式申告请交由税理士确认。</div>
    `;
    ...
    <div className="app">
      <aside>
        <div className="brand">
          ...
          <div>
            <b>豪嘉株式会社</b>
            <span>GOUKA ERP Enterprise 4.0</span>
          </div>
        </div>
  ...
  return (
    <section className="v3-dashboard">
      <div className="v3-hero">
        <div>
          <span className="v3-kicker">GOUKA ERP Enterprise 4.0</span>
      ...
      <div className="panel wide">
        <h2>经营提醒</h2>
        <p>V7.11新增今日经营、库存预警、品牌利润排行、供应商利润排行。Enterprise 4.0：优化出品管理优先级、统一图片尺寸、菜单按日常业务重新排序；保留云端同步、PDF、古物台账和全页面图片。</p>
      </div>
    </section>
  );
}
...
function ListingManagement({ items, updateListingItem, editItem, setPreviewImage, setPreviewScale }) {
  ...
  return (
    <div className="panel">
      ...
      <p className="note">Enterprise 4.0：出品管理优先优化。平台支持 NBAA / OBA / ECO Ring / JBA / AUCNET / Star Buyers / Mercari / Yahoo / 楽天 / 店铺，也可以手动输入。可快速把已入库商品加入待出品。</p>
...
function PdfExportPanel({ items, totals, exportInventoryPdf }) {
  ...
  return (
    <div className="panel">
      ...
      <p className="note">Enterprise 4.0 PDF：统一企业页眉、公司印章、適格請求書登録番号、古物商許可番号。支持全部导出、勾选导出、按日期导出。</p>
        ...
        <table>
          ...
          <tbody>
            {pageRows.length ? pageRows.map((r, i) => (
              ...
              <tr>
                <td colSpan={headers.length || 1} className="table-empty">暂无数据。商品录入、状态变更或筛选调整后会显示在这里。</td>
              </tr>
            )}
          </tbody>
        </table>
