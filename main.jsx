...
<div class="pdf-page">${bodyHtml}</div>
  ...
  function exportItemPdf(item) {
    ...
    const body = `
      <div class="pdf-header">
        <div>
          <h1>商品资料 PDF</h1>
          <p>Product Sheet / GOUKA ERP Enterprise 2.0.1.1</p>
        </div>
        <div class="company">
          <b>豪嘉株式会社</b><br />
          GOUKA INC.<br />
  ...
  function exportInventoryPdf(targetItems = null, label = "库存报告") {
    ...
    const body = `
      ...
      <div class="footer">GOUKA ERP Enterprise 2.0.1.1 / Generated by GOUKA ERP. 金额与消费税为内部参考，正式申告请交由税理士确认。</div>
    `;
    ...
    <div className="app">
      <aside>
        <div className="brand">
          ...
          <div>
            <b>豪嘉株式会社</b>
            <span>GOUKA ERP Enterprise 2.0.1.1</span>
          </div>
        </div>
      ...
      <main>
        <header>
          <div>
            <h1>豪嘉ERP Enterprise 2.0.1 Auction Engine</h1>
  ...
  return (
    <section className="v3-dashboard">
      <div className="v3-hero">
        <div>
          <span className="v3-kicker">GOUKA ERP Enterprise 2.0.1.1</span>
          <h1>经营驾驶舱 · Enterprise 2.0.1</h1>
      ...
      <div className="panel wide">
        <h2>经营提醒</h2>
        <p>V7.11新增今日经营、库存预警、品牌利润排行、供应商利润排行。Enterprise 2.0.1：优化出品管理优先级、统一图片尺寸、菜单按日常业务重新排序；保留云端同步、PDF、古物台账和全页面图片。</p>
      </div>
    </section>
  );
}
...
function AuctionSettlementBox({ form, setForm }) {
  ...
  return (
    <div className="full panel" style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", padding: "16px" }}>
      <h3 style={{ marginTop: 0 }}>🏛 日本拍卖结算（结构化数据 / Enterprise 2.0.1）</h3>
              ...
              return (
                <div className="panel" style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", marginTop: 12 }}>
                  ...
                  <div className="grid4">
                    ...
                    <Card icon={<Calculator />} title="库存真实成本" value={jpy(a.inventoryCost)} />
                    <Card icon={<Calculator />} title="消費税控除参考" value={jpy(a.taxCredit)} />
                  </div>
                  <p className="note">成本规则：库存真实成本 = 落札金額 + 落札手数料 + 手数料消費税 + 国内送料。消费税控除参考 = 落札消費税 + 手数料消費税。</p>
                </div>
              );
            })() : (
...
function ListingManagement({ items, updateListingItem, editItem, setPreviewImage, setPreviewScale }) {
  ...
  return (
    <div className="panel">
      ...
      <p className="note">Enterprise 2.0.1：出品管理优先优化。平台支持 NBAA / OBA / ECO Ring / JBA / AUCNET / Star Buyers / Mercari / Yahoo / 楽天 / 店铺，也可以手动输入。可快速把已入库商品加入待出品。</p>
