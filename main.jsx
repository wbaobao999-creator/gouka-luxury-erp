...
function App() {
  ...
  function exportBackup() {
    const data = { version: "GOUKA-ERP-V6.6556", exportedAt: new Date().toISOString(), items };
    ...
    <div className="app">
      <aside>
        <div className="brand">
          ...
          <div>
            <b>豪嘉株式会社</b>
            <span>GOUKA Luxury ERP V6.6556</span>
          </div>
        </div>
  ...
  return (
    <section className="v3-dashboard">
      <div className="v3-hero">
        <div>
          <span className="v3-kicker">GOUKA ERP V6.6556</span>
          <h1>经营驾驶舱</h1>
      ...
      <div className="panel wide">
        <h2>经营提醒</h2>
        <p>V6.6556新增今日经营、库存预警、品牌利润排行、供应商利润排行。下一阶段可接Supabase，实现多电脑同步和图片云存储。</p>
      </div>
    </section>
  );
}
...
function BackupPanel({ items, exportBackup, importBackup }) {
  return (
    <div className="panel">
      ...
      <p className="note">当前系统数据保存在本机浏览器。V6.6556备份会包含商品、字典、供应商、现金流。换电脑、清理浏览器、重装系统前，一定要先导出备份。</p>
...
function AiAssistant({ onApplyDraft, dictionaries, suppliers }) {
  ...
  return (
    <div className="panel">
      <h2>🤖 AI录入助手 V6.6556</h2>
      <p className="note">
        V6.6556新增图片上传通道。可以上传商品图、发票图、拍卖截图并预览；识别文字仍需粘贴或人工补充。
        确认后图片会一起带入商品录入页。
      </p>
...
function SupplierPanel({ suppliers, setSuppliers, downloadCSV }) {
  ...
  return (
    <div className="panel">
      ...
      <p className="note">
        V6.6556新增：供应商独立管理。录入商品选择供应商后，会自动带出地址与备注，减少员工重复输入。
      </p>
...
function DictionaryPanel({ dictionaries, setDictionaries }) {
  ...
  return (
    <div className="panel">
      ...
      <p className="note">
        V6.6556开始，品牌、商品名、材质、颜色、产地、来源、平台都可以在这里维护。
